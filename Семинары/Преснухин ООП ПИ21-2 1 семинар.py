...
class Line(Field):
    ...
    def length(self):
        ...
        l = sqrt((x_range**2 if x_range != 1 else 0) +                  (y_range**2 if y_range != 1 else 0))
        return l
...
class Rectangle(Sharp): # класс прямоугольника 
    ...
    def __str__(self):
        return f"Это прямоугольник с левой верхней точкой ({self.__x}, {self.__y}), шириной {self.__width} и высотой {self.__height}"
...
class Square(Rectangle): # класс квадрата
    ...
    def __str__(self):
        return f"Это квадрат с левой верхней точкой ({self._Rectangle__x}, {self._Rectangle__y}) и стороной {self._Rectangle__width}"
